...
const optionNull = (arrayLinks) => {
    ...
    if (arrayLinks.length > 0) {
        arrayLinks.forEach(element => {
            printLinks +=
                `
            ${chalk.green('PATH: ')} ${chalk.yellow(element.file)}
            ${chalk.green('LINK: ')} ${chalk.yellow(element.href)}
            ${chalk.green('TEXT: ')} ${chalk.yellow(element.text.substring(0, 50))}
            `;
        });
    } else {
        return chalk.bgYellow.blackBright('NO SE ENCONTRO LINKS!');
    }
    return printLinks;
};
...
const optionStastValidate = (arrayLinks) => {
    if (arrayLinks.length > 0) {
        ...
        const printLinks =
            ...
            ${chalk.greenBright('TOTAL: ')} ${chalk.yellowBright(arrayLinks.length)}
            ${chalk.greenBright('LINK: ')} ${chalk.yellowBright(numLinks.length)}
            ${chalk.greenBright('BROKEN: ')} ${chalk.yellowBright(numBroken.length)}
            `;
        return printLinks;
    } else {
        const printLinks =
            ...
            ${chalk.greenBright('TOTAL: ')} ${chalk.yellowBright(0)}
            ${chalk.greenBright('LINK: ')} ${chalk.yellowBright(0)}
            ${chalk.greenBright('BROKEN: ')} ${chalk.yellowBright(0)}              
            `;
        return printLinks;
    }

};
// console.log(optionStastValidate(prueba));
